...
bool minCellOK(void){
    ...
    return (cellstats.mincell_mV > dynamic_offset_mV_load && cellstats.mincell_mV > CELL_CUTOUT_VOLTAGE_DISCHARGE_mV);
}
...
bool chargeTempCheck(void){
    ...
    result &= (isl_int_temp > MIN_CHARGE_TEMP_C);
    result &= (thermistor_temp > MIN_CHARGE_TEMP_C);
...
void setErrorReasonFlags(volatile error_reason_t *datastore){
    ...
    datastore->UNDERTEMP_FLAG |= !((state != CHARGING && isl_int_temp > MIN_TEMP_C) || (state == CHARGING && isl_int_temp > MIN_CHARGE_TEMP_C));
    datastore->UNDERTEMP_FLAG |= !((state != CHARGING && thermistor_temp > MIN_TEMP_C) || (state == CHARGING && thermistor_temp > MIN_CHARGE_TEMP_C));
