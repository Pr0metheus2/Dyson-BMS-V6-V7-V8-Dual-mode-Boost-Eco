...
// Define global variables declared as extern in main.h
...
uint16_t MAX_CHARGE_CELL_VOLTAGE_mV = 4150;

static uint8_t charge_state_modify_flag = 1; // 1=force set, 2=sticky-lowest (discharge), 3=sticky-highest (charging)
static uint8_t charge_state_memory = 3; // stores the last charge level to compare
...
uint8_t CalculateChargeIndicator(void){
    ...
    if (charge_state_memory >= 3) return 3;
    if (charge_state_memory == 2) return 2;
    return 1; // Covers 0 and 1
}
...
__EEPROM_DATA(0, 0, 0x2E, 0xE0, 0, 0, 0x2E, 0x2E);    // Space to store 32bit uOhm current internal resistence of min cell (0x20) and overall greatest current internal resistence of min cell (0x24), EEPROM addresses 0x20 - 0x27
...
__EEPROM_DATA(0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0, 0); //(0x48 - 0x4F) Cell voltage offsets to be set by user: Voff1: 0x48, Voff2: 0x49, Voff3: 0x4A, Voff4: 0x4B, Voff5: 0x4C, Voff6: 0x4D, 
...
//INIT STEPS
    ...
    mincell_internal_resistence_last_charge_uOhms = Read32BitUintVariableFromEEPROM(EEPROM_ADDR_MIN_CELL_IR_LAST);
    packdelta_end_of_charging_wait_mV = Read16BitUintVariableFromEEPROM(EEPROM_ADDR_PACK_DELTA_END_CHARGE);
    mincell_internal_resistence_max = Read32BitUintVariableFromEEPROM(EEPROM_ADDR_MIN_CELL_IR_MAX);
    thermistor_temp_max = DATAEE_ReadByte(EEPROM_ADDR_THERMISTOR_TEMP_MAX);
    isl_int_temp_max = (uint16_t) DATAEE_ReadByte(EEPROM_ADDR_ISL_INT_TEMP_MAX);
    cell_offset_voltage_1 = (int8_t) DATAEE_ReadByte(EEPROM_ADDR_CELL_OFFSET_1);
    cell_offset_voltage_2 = (int8_t) DATAEE_ReadByte(EEPROM_ADDR_CELL_OFFSET_2);
    cell_offset_voltage_3 = (int8_t) DATAEE_ReadByte(EEPROM_ADDR_CELL_OFFSET_3);
    cell_offset_voltage_4 = (int8_t) DATAEE_ReadByte(EEPROM_ADDR_CELL_OFFSET_4);
    cell_offset_voltage_5 = (int8_t) DATAEE_ReadByte(EEPROM_ADDR_CELL_OFFSET_5);
    cell_offset_voltage_6 = (int8_t) DATAEE_ReadByte(EEPROM_ADDR_CELL_OFFSET_6);
    MAX_CHARGE_CELL_VOLTAGE_mV = Read16BitUintVariableFromEEPROM(EEPROM_ADDR_MAX_CHARGE_VOLT);
    if (MAX_CHARGE_CELL_VOLTAGE_mV > MAX_CHARGE_CELL_VOLTAGE_BOOST_mV || MAX_CHARGE_CELL_VOLTAGE_mV < 3600){ //To be sure it never reads garbage date from EEPROM
        MAX_CHARGE_CELL_VOLTAGE_mV = MAX_CHARGE_CELL_VOLTAGE_BOOST_mV;
    }
    
    // Initialize detect history to avoid green flash on wake up
    uint16_t initial_detect = readADCmV(ADC_CHRG_TRIG_DETECT);
    for(uint8_t i=0; i<5; i++) {
        adc_chrg_trig_detect_voltage_history[i] = initial_detect;
    }
    
    // Initialize PWM for breathing LED
    CCP1CON = 0x0C; // PWM mode; P1A, P1C active-high; P1B, P1D active-high
    CCPR1L = 0x00;  // Start with 0 duty cycle
    PSTR1CON = 0x00; // Disable steering by default
    
    charge_state_modify_flag = 1; // Force set first value
    charge_state_memory = 3; // Start high
...
void sleep_ISL_or_PIC(void){
    ...
    if (detect == CHARGER){
        ...
        Write32BitUintVariableToEEPROM(EEPROM_ADDR_MIN_CELL_IR_LAST, mincell_internal_resistence_last_charge_uOhms);
        if (mincell_internal_resistence_max > Read32BitUintVariableFromEEPROM(EEPROM_ADDR_MIN_CELL_IR_MAX)){
            Write32BitUintVariableToEEPROM(EEPROM_ADDR_MIN_CELL_IR_MAX, mincell_internal_resistence_max);
            Write16BitUintVariableToEEPROM(EEPROM_ADDR_MIN_CELL_V_LAST_TRIG, mincell_voltage_mV_last_trigger_eeprom);
            Write16BitUintVariableToEEPROM(EEPROM_ADDR_MIN_CELL_V_LAST_TRIG_RECOV, mincell_voltage_mV_last_trigger_recover_eeprom);
            Write16BitUintVariableToEEPROM(EEPROM_ADDR_DISCHARGE_CURR_LAST_TRIG, discharge_current_mA_last_trigger_eeprom);
        }
        if (thermistor_temp_max > DATAEE_ReadByte(EEPROM_ADDR_THERMISTOR_TEMP_MAX)){
            DATAEE_WriteByte(EEPROM_ADDR_THERMISTOR_TEMP_MAX, thermistor_temp_max);
        }
        if ((uint8_t) isl_int_temp_max > DATAEE_ReadByte(EEPROM_ADDR_ISL_INT_TEMP_MAX)){
            DATAEE_WriteByte(EEPROM_ADDR_ISL_INT_TEMP_MAX, (uint8_t) isl_int_temp_max);
        }
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_PACK_DELTA_END_CHARGE, packdelta_end_of_charging_wait_mV);
        
        //Write current cell voltages (including applied offsets) to be compared with multimeter readings
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_CELL_V1_SLEEP, CellVoltages[1]);
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_CELL_V2_SLEEP, CellVoltages[2]);
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_CELL_V3_SLEEP, CellVoltages[3]);
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_CELL_V4_SLEEP, CellVoltages[4]);
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_CELL_V5_SLEEP, CellVoltages[5]);
        Write16BitUintVariableToEEPROM(EEPROM_ADDR_CELL_V6_SLEEP, CellVoltages[6]);
        ...
        ISL_SetSpecificBits(ISL.SLEEP, 1);   // only one sleep command to ISL is necessary because we do not let the ISL sleep while on charger. It is stated in the data sheet that ISL will only go to sleep in another changer of sleep bit if WKUP is pulled high. We dont need this anymore
...
void idle(void){
    ...
    if (detect == CHARGER && !CheckStateInDetectHistory(CHARGER) && maxCellOK()){
        charge_complete_flag = false;
        show_cell_delta_LEDs = 1;
        charge_state_modify_flag = 1; // Resample
    }
    ...
    if (detect == TRIGGER                       //Trigger is pulled
        ...
        if ((previous_detect_was_charger && cellDeltaLEDIndicator())
            ...
            bool charger_removed_after_full_charge = previous_detect_was_charger;
            previous_detect_was_charger = false;                            //Then remove flag
            if (charger_removed_after_full_charge){
                charge_state_modify_flag = 1; // Resample before sleep
                state = SLEEP;    //After charger removal and imbalance indication, go straight to sleep.
            }
            else{
    ...
    else if (detect == CHARGER && ISL_GetSpecificBits_cached(ISL.WKUP_STATUS)){    //Pack on charger but we didn't start charging = Yellow LED
    ...
    //Mode-dependent threshold for resuming charging
    uint16_t resume_charge_thresh = (MAX_CHARGE_CELL_VOLTAGE_mV == MAX_CHARGE_CELL_VOLTAGE_BOOST_mV) ? PACK_CHARGE_NOT_COMPLETE_BOOST_THRESH_mV : PACK_CHARGE_NOT_COMPLETE_ECO_THRESH_mV; //If the max cell voltage is below "resume_charge_thresh" and the pack is marked as fully charged, unmark it as charged (only if not still on charger; to prevent infinite charge loop).


    if (charge_complete_flag == true && cellstats.maxcell_mV < resume_charge_thresh && detect != CHARGER){      //If the max cell voltage is below threshold and the pack is marked as fully charged, unmark it as charged (only if not still on charger; to prevent infinite charge loop).
        charge_complete_flag = false;
        show_cell_delta_LEDs = 1;
        charge_state_modify_flag = 1; // Resample
    }
...
void charging(void){
    ...
    if (!ISL_GetSpecificBits_cached(ISL.ENABLE_CHARGE_FET)     //if we aren't already charging
        ...
        resetLEDBlinkPattern();
        // Set_LED_RGB(0b001); //Blue LED - Commented out to prevent flash before breathing mode starts
        charge_state_modify_flag = 3; // sticky-highest during charge
    }
    ...
    else if (!safetyChecks()){     //There was an error
        ISL_SetSpecificBits(ISL.ENABLE_CHARGE_FET, 0); //Disable Charging
        charge_duration_counter.enable = false;         //Stop charge timer
        state = ERROR;
    }
    else if (!chargeTempCheck()){ //Temperature out of range during charging
        ISL_SetSpecificBits(ISL.ENABLE_CHARGE_FET, 0); //Disable Charging
        charge_duration_counter.enable = false;         //Stop charge timer
        state = CHARGING_WAIT;
    }
...
void chargingWait(void){
    if(!(isl_int_temp > MIN_CHARGE_START_TEMP_C && thermistor_temp > MIN_CHARGE_START_TEMP_C && isl_int_temp < MAX_CHARGE_START_TEMP_C && thermistor_temp < MAX_CHARGE_START_TEMP_C)){
        ...
        Set_LED_RGB(0b110); //Yellow LED indicating wait for temperature to normalise
    }
        ...
        Set_LED_RGB(0b111); //White LED indicating charing wait
    }
    ...
    static bool need_to_clear_LEDs_for_cell_voltage_indicator = 1;
        ...
        if (!ISL_GetSpecificBits_cached(ISL.ENABLE_DISCHARGE_FET)  //If discharge isn't already enabled
            ...
            && safetyChecks()
                ...
                charge_state_modify_flag = 2; // sticky-lowest during discharge
        }
		...
		&& (minCellOK() || (onetime_runtime_counter.enable && onetime_runtime_counter.value < 64)) //allow 2 seconds, discharge currrent measurement to stabilise
            && safetyChecks()
                ...
                if (onetime_runtime_counter.value > 156 && discharge_current_mA >= 4000){ //output must run at least 5 seconds, same discharge current requirement as in idle for calculation
                ...
                runonce = 0;
                Set_LED_RGB(0b001);       //Set LED to blue
        }
        else if (!minCellOK() && ISL_GetSpecificBits_cached(ISL.ENABLE_DISCHARGE_FET) && onetime_runtime_counter.enable && onetime_runtime_counter.value >= 64){   //If we hit the min cell voltage cut off, prevent discharging battery further until it is put on charger, but allow 2 seconds, discharge currrent measurement to stabilise
            full_discharge_flag = true;
            state = IDLE;
        }     
        else if (!safetyChecks()){
                state = ERROR;
        } else if (detect == CHARGER && need_to_clear_LEDs_for_cell_voltage_indicator == 1){    //Charger attached while trigger was pulled
			...
			ledBlinkpattern (num_blinks, 0b111, 100, 100, 500, 0);
			if (LED_code_cycle_counter.value > 1){	//One LED cycle completed
				...
				charge_complete_flag = false;
				// resetLEDBlinkPattern();
				// nonblocking_wait_counter.value = 0; //Reset pause timer
				// nonblocking_wait_counter.enable = true;
                charge_state_modify_flag = 1; // Force set
				state = IDLE;
			}
        }
            ...
            if (cellVoltageLEDIndicator()){   //Repeat the cellVoltageLEDIndicator() function until one iteration, when it will return true.
                charge_state_modify_flag = 1; // Resample before sleep
                state = SLEEP;   //Skip the normal idle-green hold here and go straight to sleep after saving state.
            }
        }
...
void error(void){
    ...
    if (critical_i2c_error || past_error_reason.ISL_BROWN_OUT){         
        ...
        if (recovery_attempts < 3){
            ...
            if (ISL_GetSpecificBits_cached(ISL.WKPOL)){
                past_error_reason = (error_reason_t){0};
                current_error_reason = (error_reason_t){0};
                critical_i2c_error = false;
                recovery_attempts = 0;
                state = IDLE;
                return;
            }
             ...
             while(1){	 //It's called critical for a reason
                ...
                if (!detect) LED_code_cycle_counter.enable = true;	//Start led error code sequence after trigger released
                else { LED_code_cycle_counter.value = 0; LED_code_cycle_counter.enable = false; }	//Attaching charger or pulling trigger will reset LED code count

                if (LED_code_cycle_counter.value > NUM_OF_LED_CODES_AFTER_FAULT_CLEAR) RESET();	//Once required number of error codes are shown, use the nuclear option.
                
                if (past_error_reason.ISL_BROWN_OUT) ledBlinkpattern(16, 0b100, 500, 500, 1000, 1000);	//ISL brown out
                else ledBlinkpattern(15, 0b100, 500, 500, 1000, 1000);	//critical i2c error
    ...
    else if (past_error_reason.CHARGE_OC_FLAG) errorBlinkpattern (8);                       //Charge overcurrent
    else if (past_error_reason.DISCHARGE_OC_FLAG) errorBlinkpattern (9);                    //Discharge overcurrent
    else if (past_error_reason.DISCHARGE_SC_FLAG) errorBlinkpattern (10);                   //Discharge short circuit
    else if (past_error_reason.DISCHARGE_OC_SHUNT_PICREAD) errorBlinkpattern (11);          //Discharge overcurrent (PIC reading)
    else if (past_error_reason.CHARGE_ISL_INT_OVERTEMP_PICREAD) errorBlinkpattern (12);     //Charge internal overtemp (PIC reading)
    else if (past_error_reason.CHARGE_THERMISTOR_OVERTEMP_PICREAD) errorBlinkpattern (13);  //Charge thermistor overtemp (PIC reading)
    else if (past_error_reason.UNDERTEMP_FLAG) errorBlinkpattern (14);                      //Undertemp
    else if (full_discharge_flag) ledBlinkpattern (3, 0b001, 300, 300, 750, 750);			//trigger is pulled but battery is low
    else errorBlinkpattern (20);															//Unidentified Error
...
{    
    ...
    while (1)
    {
        ...
        if (TMR4_HasOverflowOccured()){         //Every 32ms 
            ...
            if (state == CHARGING){
                ledBreathingUpdate(0b001); // Blue breathing
            }
