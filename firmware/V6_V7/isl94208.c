...
void ISL_Init(void){ 
    ...
    /* 0 = Auto OC discharge control enabled
     ...
     10 = 2mOhm shunt @ 650mV SC threshold = 325A short circuit trip. SC trips were occuring at 100A setting. Current probe measured ~120A peaks at startup, old setting of 350mV was marginal. Would trip with new cells on V8 (low setting)
     00 = Overcurrent timeout 160ms or 2.5ms if discharge time divider set. 2.5ms selected. Can't set any lower.*/
    ISL_Write_Register(DischargeSet, 0b00001000);
     ...
     */
    ISL_Write_Register(ChargeSet, 0b01001101);
...
uint16_t ISL_GetAnalogOutmV(isl_analogout_t value){
    ...
    __delay_us(1);  //Wait a little bit 1�s
...
void ISL_ReadAllCellVoltages(void){
    ...
    CellVoltageHistory[OldestVoltageIndex][1] = ISL_GetAnalogOutmV(AO_VCELL1)*2 + cell_offset_voltage_1; //Cell voltages have to be multiplied by two since ISL scales them down by two.
    CellVoltageHistory[OldestVoltageIndex][2] = ISL_GetAnalogOutmV(AO_VCELL2)*2 + cell_offset_voltage_2;
    CellVoltageHistory[OldestVoltageIndex][3] = ISL_GetAnalogOutmV(AO_VCELL3)*2 + cell_offset_voltage_3;
    CellVoltageHistory[OldestVoltageIndex][4] = ISL_GetAnalogOutmV(AO_VCELL4)*2 + cell_offset_voltage_4;
    CellVoltageHistory[OldestVoltageIndex][5] = ISL_GetAnalogOutmV(AO_VCELL5)*2 + cell_offset_voltage_5;
    CellVoltageHistory[OldestVoltageIndex][6] = ISL_GetAnalogOutmV(AO_VCELL6)*2 + cell_offset_voltage_6;
