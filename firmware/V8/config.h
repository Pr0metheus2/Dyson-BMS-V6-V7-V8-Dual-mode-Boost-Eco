...
const uint16_t PACK_CHARGE_NOT_COMPLETE_THRESH_BOOST_mV = 4050; // Resume threshold for Boost
const uint16_t PACK_CHARGE_NOT_COMPLETE_THRESH_ECO_mV = 3900;   // Resume threshold for Eco. If the max cell voltage goes under this threshold while the charger is connected but idle, the charge_complete_flag will be cleared and charging will restart.
...
#define ASCII_FIRMWARE_VERSION 0x35;      //ASCII '1' = 0x31. This must be manually determined so the EEPROM section is human readable.
...
/* Length of time to wait while idle before going to sleep
 469*32ms = 15.008s */
const uint16_t IDLE_SLEEP_TIMEOUT = 469;
